...
export class CommandService {
  ...
  async createLabyrinth(userId): Promise<boolean> {
    ...
    return this.labyrinthRepositoryRepository.create(this.labyrinthDataMapperDataMapper.toDalEntity(userInstance))
  }

  async setLabyrinthStartPoint(id: string, x: number, y: number, userId: string, version: number) {
    const labyrinthSchema = await this.labyrinthRepositoryRepository.getByIdAndUserId(id, userId)
    ...
    return this.labyrinthRepositoryRepository.replace(
      ...
      this.labyrinthDataMapperDataMapper.toDalEntity(labyrinthInstance), version)
    
  }

  async setLabyrinthEndPoint(id: string, x: number, y: number, userId: string, version: number) {
    const labyrinthSchema = await this.labyrinthRepositoryRepository.getByIdAndUserId(id, userId)
    if (!labyrinthSchema) {
  ...
  async setLabyrinthPlayfieldPoint(id: string, x: number, y: number, type: LabyrinthFulfillmentType, userId: string, version: number) {
    const labyrinthSchema = await this.labyrinthRepositoryRepository.getByIdAndUserId(id, userId)
    if (!labyrinthSchema) {
      throw Error('labyrinth not found!')
    }
  ...
  async createUserOnTheFly(username: string, password: string) {
    ...
    if (!UserSchema) {
      ...
      await this.userRepository.create(this.userDataMapperDataMapper.toDalEntity(userInstance))
    }
    return true
  }
}
